...
//when we use a, substitues with the value of the memory address it points to. i.e. int 0x0030[5]; 
```
...
* function calls require a jump to a different area of assessembly (perhaps from an external library, ...) as "call"
...
*function pointer* holds an address value that refers to an address of memory with execulateable code (m. first instruction of the function call)
* usecases: do something, then call this function
* if error, call function
* give a function to a datasource to give updates?
...
fptr();
```
* in assesembly, it dereferences the address
...
* application level interuptions
...
//this asks compiler to not optomize this type 
...
int main()
{
	...
	//c library event: if SIGINT (interuppted), call variable
...
* `signhandler_t` is a typedef of `typedef void(*sighandler_t)(int);`
...
* system call functions operate on file descripters
...
```c
errno = 0;
//read 100char from stin into array "buffer"
...
* parallelism, occuring! describing the state and execution of a currently running program

*process* collection of data, instructions, and state to run a computer program

* OS requres a process: avaliable memory, what has been used, program counter, what code ... (needed! additional software to manage this, a *necessary abstration* to mediate between program -> hardware devices)
...
* in stack, variable -> referes to someone in heap, static, instructions in static -> code
* code like fopen() --> libc in static --> (new section) kernel space

*kernel space* copy of the operating systems management data, and what the OS is doing (e.g. addres of a stdout device)
...
*process indentifier* unique associated number
*parent process*
...
![there is a seperation of space](2017/201713.png)
...
if execl is not successful it, evaulation of the expression happens after it --> result is never set if its successful; everything is gone!
--> if its unsuccessful, it returns back -1 (we have the resouces, need to deallocate memory)

`pid_t fork()` clone a child process that is a copy of the memory image of the parent
* the process identifer (pid) is different
...
* both programs will run in parallel (but actually the process scheduler runs in a specfic order)
...
* the sleep enforces some consistancy?
...
* signals allow you to do syncronisation, can send an integer
...
*syncrononous* a->b, b->a, like a phone call
*asyncrononous* usb -> interupt -> reaction, inserting a usb device
...
* can make new pipes from subchannel to communicte seperately between processes, as many you like
...
`select()` moniters file descriptors for events
...
* the pipe can read from stdinput [0], stdout, stderr ==> automatically created for every process
* non blocking operations need to look at flags to see if input been made

--> nb_select_pipe is where child can keep doing shit after sending

#WEEK 9 THREADS
* communicate between two programs at same time (with independant, duplicated memory); IPC: signals, pipes, shared memory
*parallel* same thing happening between two processes
*concurrency* multiple processes working together (like table tennis)

*threads* parallelism (as opposed to sequentialism) using the same memory image
* threads with a unique program counter
* tasks have *dependancies*

*task parallelism* different tasks performed by different processes

![task dependancy graph](2017/201720.png)
...
* writing to same file with multiple threads, need a way to preallocate filesize

`pthread_create(&pthread_struct, NULL, function pointer to start with, return value)`
* continuation is independant (but there is threadsafety, i.e. print prints out the whole thing before doing the action of the other process)
...
* having multiple threads increaseing the same number can have an issue, because you gotta read -> compute -> write, and in that time other things can happen in other threads
...
* only one thread can acccess the code, the other thread performs a blocking operation 
* everytime you lock gotta unlock later (or deadlock)
...
* we want to serial the access to certain segments to prevent race-conditions from happening [otherwise we can parraellise, when the data cant corrupt]
...
* always aquire things in the same order: such that A->B is required
...
3. no preemtion (no one tells the process to give up a resource)
...
*starvation* the thread is never able to aquire both resources, imbalance of executing different tasks over the threads
* the order of unlocking is unimportant

---
*locking granulaity* the amount of data that a lock protects
...
* this guarentees the link hasnt been modified during the code running phase (e.g. node.next changes)
